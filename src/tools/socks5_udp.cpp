                               ...
                               const char *socks5_server, uint16_t socks5_port) noexcept
{
    ...
    const uint16_t dport = ntohs(udph->uh_dport);
    if (dport == 0) {
        return false;
    }

    const uint32_t daddr = iph->daddr;

    auto found = dest_to_sock.find(daddr);
    if (found != dest_to_sock.end())  {
        ::socks5_send_udp_packet(found->second.second.m_fdSocket, buffer, size);
        return true;
    }

    const int fdSoc = sock_utils::create_tcp_socket_client(socks5_server, socks5_port);
    if (fdSoc == 0) {
	...
	if ( udp_conn.second == 0 ) {
        std::perror("socks5_udp_connection_request failed");
        exit(EXIT_FAILURE);
	}

    std::cout << "UDP ADDRESS AND BND.PORT: \t" << inet_ntoa(udp_conn.first) << ":" << udp_conn.second << std::endl;
    const int fdSocUdp = sock_utils::create_udp_socket(&udp_conn.first, udp_conn.second);
    if (fdSocUdp == -1) {
        std::perror("sock_utils::create_udp_socket failed");
		exit(EXIT_FAILURE);
    }
    ...
    ::socks5_send_udp_packet(fdSocUdp, buffer, size);
    socks5_upd_socket udp_socket;
    udp_socket.m_fdSocket = fdSocUdp;
    dest_to_sock[daddr] = std::make_pair(fdSoc, udp_socket);
    new_udp_sock = fdSocUdp;
