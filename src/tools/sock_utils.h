...
{
    inline void get_socket_info(int fdSoc, struct sockaddr_in &addr) noexcept
    {
    ...
    inline void print_socket_info(int fdSoc) noexcept
    {
    ...
    inline void check_socket(int fdSoc) noexcept
    {
        int error = 0;
    ...
    inline int create_udp_socket(struct in_addr *inp, uint16_t dport) noexcept
    {
    ...
    inline int read_data(int fdSoc, char *buffer, size_t buff_read_len, int recv_flag) noexcept
    {
    ...
    inline int write_data(int fdSoc, const char *buffer, size_t buff_write_len, int send_flags) noexcept
    {
    ...
    inline static int close_connection(int fdSoc) noexcept
    {
        const int close_ret = ::close(fdSoc);
        return close_ret;
    }

    inline int create_tcp_socket_client(const char *name, std::uint16_t port) noexcept
    {
        hostent *hoste;
        sockaddr_in addr;
